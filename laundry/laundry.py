...
BASE_URL_QUERY = 'http://gmu.esuds.net/\
        RoomStatus/machineStatus.i?bottomLocationId='

def load_data(hall):
    """Extract table data from esuds for specified hall"""
    html = urlopen(BASE_URL_QUERY+str(hall.location_id)).read()
...
def generate_current_chart(filepath, hall):
    ...
    """
    custom_style = Style(colors=('#B6E354', '#FF5995', '#FEED6C'))
    chart = pygal.StackedBar(style=custom_style)
    chart.title = 'Current laundry machine usage in Whitetop Hall'
    chart.x_labels = ['Washers', 'Dryers']
    chart.add('Available', get_num_machines_per_status(AVAILABLE))
    chart.add('In Use', get_num_machines_per_status(IN_USE))
    chart.add('Cycle Complete', get_num_machines_per_status(CYCLE_COMPLETE))
